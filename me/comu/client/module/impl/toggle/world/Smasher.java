...
public final class Smasher extends ToggleableModule
{
    private final EnumProperty<Mode> mode = new EnumProperty<>(Mode.INSTANT, "Mode", "m");
    private final Property<Boolean> redstone = new Property<>(true, "Redstone", "red","repeaters","redstonerepeaters","comparators","redstonecomparators"),
        plants = new Property<>(true, "Plants","plant","grass"),
        torches = new Property<>(true, "Torches","torch","redstonetorches","rtorches");
    ...
    public Smasher()
    {
        super("Smasher", new String[] {"smasher","smash"}, 0xFF1733, ModuleType.WORLD);
        this.offerProperties(redstone, plants, torches, silent, box);
 ...
 if (this.mode.getValue() == Mode.INSTANT)
        {
            ...
            if  (plants.getValue()){
                if (target instanceof BlockFlower || target instanceof BlockRedFlower || target instanceof BlockYellowFlower || target instanceof BlockTallGrass || target instanceof BlockDoublePlant || target instanceof BlockCrops || target instanceof BlockFlower || target instanceof BlockMushroom || target instanceof  BlockNetherWart || target instanceof BlockReed || target instanceof BlockLilyPad || target instanceof BlockDeadBush/*|| target instanceof     */) {
                    valid = true;
                }
            }
     if  (torches.getValue()){
         if (target instanceof BlockTorch || target instanceof BlockRedstoneTorch) {
             valid = true;
         }
         ...
         if (redstone.getValue()) {
             if (target instanceof BlockRedstoneTorch || target instanceof BlockRedstoneWire || target instanceof BlockRedstoneComparator|| target instanceof BlockRedstoneRepeater ) {
                 valid = true;
             }
         }
     }
