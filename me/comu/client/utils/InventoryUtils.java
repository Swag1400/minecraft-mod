...
public class InventoryUtils
{
    ...
    public static void useFirst(final Item item)
    {
        for (int index = 0; index <= 8; ++index)
        {
            ...
            if (stack != null && stack.getItem() == item)
            {
                ...
                Helper.mc().func_175102_a().addToSendQueue(new C09PacketHeldItemChange(index));
                Helper.mc().func_175102_a().addToSendQueue(new C08PacketPlayerBlockPlacement(Helper.mc().thePlayer.inventory.getCurrentItem()));
                Helper.mc().thePlayer.stopUsingItem();
                Helper.mc().func_175102_a().addToSendQueue(new C09PacketHeldItemChange(oldItem));
                break;
            }
        }
    }
    ...
    public static void instantUseFirst(final Item item)
    {
        for (int index = 0; index <= 8; ++index)
        {
            ...
            if (stack != null && stack.getItem() == item)
            {
                ...
                for (int x = 0; x <= 32; ++x)
                {
                    Helper.mc().func_175102_a().addToSendQueue(new C03PacketPlayer(Helper.mc().thePlayer.onGround));
                }

                Helper.mc().func_175102_a().addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
                Helper.mc().thePlayer.stopUsingItem();
                Helper.mc().func_175102_a().addToSendQueue(new C09PacketHeldItemChange(oldItem));
                break;
            }
    ...
    public static boolean isSoup(final ItemStack stack)
    {
        if (stack != null)
        {
    ...
    public static boolean inventoryHasSoup()
    {
    	for(int index = 0; index <= 36; ++index)
    	{
    		final ItemStack stack = Helper.mc().thePlayer.inventory.getStackInSlot(index);
    		
    		if (stack != null && isSoup(stack))
    		{
    			return true;
    		}
    	}
