package me.comu.api.minecraft.render;
...
public final class RenderMethods
{
    ...
    public static void drawCorneredRectangle(final float x, final float y, final float w, final float h, final float thickness, final int hex) {
        final float hd = (h - y) / 4.0f;
        final float wd = (w - x) / 4.0f;
        drawRect(x - thickness, y, x, y + hd, hex);
        drawRect(x - thickness, y - thickness, x + wd, y, hex);
        drawRect(w, y, w + thickness, y + hd, hex);
        drawRect(w - wd, y - thickness, w + thickness, y, hex);
        drawRect(x - thickness, h - hd, x, h, hex);
        drawRect(x - thickness, h, x + wd, h + thickness, hex);
        drawRect(w, h - hd, w + thickness, h, hex);
        drawRect(w - wd, h, w + thickness, h + thickness, hex);
    }
    public static void drawRectCornered(final Vector4f vector, final Entity entity) {
        drawCorneredRectangle(vector.x, vector.y, vector.w, vector.h, 1.0f, -1726934767);
        if (entity instanceof EntityPlayer) {
            drawCorneredRectangle(vector.x, vector.y, vector.w, vector.h, 0.5f, Gun.getInstance().getFriendManager().isFriend(entity.getName()) ? -16734209 : (EntityHelper.isOnSameTeam(false, (EntityLivingBase)entity) ? -5767424 : -65536));
        }
        else if (entity instanceof EntityItem) {
            drawCorneredRectangle(vector.x, vector.y, vector.w, vector.h, 0.5f, -3355444);
        }
        else {
            drawCorneredRectangle(vector.x, vector.y, vector.w, vector.h, 0.5f, -65536);
        }
    }
    public static void drawRectESP(final float x, final float y, final float x1, final float y1, final int color) {
        enableGL2D();
        glColor(color);
        drawRect(x, y, x1, y1);
        disableGL2D();
