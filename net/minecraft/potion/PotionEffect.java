...
public class PotionEffect {
     ...
     */
    void setSplashPotion(boolean splashPotion) {
    ...
    private int decrementDuration() {
        PotionDecrementEvent potionDecrementEvent = new PotionDecrementEvent();
        Gun.getInstance().getEventManager().dispatch(potionDecrementEvent);

        if (potionDecrementEvent.isCancelled()) {
            return this.duration;
        }
    ...
    private void performEffect(EntityLivingBase entityIn) {
        if (this.duration > 0) {
