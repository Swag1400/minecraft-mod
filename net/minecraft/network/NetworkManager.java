...
public class NetworkManager extends SimpleChannelInboundHandler {
    ...
    public void sendPacket(Packet packetIn) {
        PacketEvent packetEvent = new PacketEvent(packetIn, PacketEvent.Status.OUTGOING);
        Gun.getInstance().getEventManager().dispatch(packetEvent);
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Object p_channelRead0_2_) {
        PacketEvent packetEvent = new PacketEvent((Packet) p_channelRead0_2_, PacketEvent.Status.INCOMING);
        Gun.getInstance().getEventManager().dispatch(packetEvent);
        if (packetEvent.isCancelled())
        {
            return;
        }
        this.channelRead0(p_channelRead0_1_, packetEvent.getPacket());
    }
