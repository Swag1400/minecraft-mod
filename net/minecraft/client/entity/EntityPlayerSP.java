...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        MotionUpdateEvent motionUpdateEvent = new MotionUpdateEvent(MotionUpdateEvent.Time.BEFORE, posX, getEntityBoundingBox().minY, posZ, isSprinting(), onGround, isSneaking(), rotationYaw, rotationPitch);
        Gun.getInstance().getEventManager().dispatch(motionUpdateEvent);
        Render rotation = (Render) Gun.getInstance().getModuleManager().getModuleByAlias("Render");
        Property<Boolean> rotations = (Property<Boolean>) rotation.getPropertyByAlias("rotations");
        if (this.mc.gameSettings.thirdPersonView != 0 && rotations.getValue()) {
            this.clientEventPitch = motionUpdateEvent.getRotationPitch();
            this.rotationYawHead = motionUpdateEvent.getRotationYaw();
            this.renderYawOffset = motionUpdateEvent.getRotationYaw();
        } else {
            this.clientEventPitch = rotationPitch;
        }
        ...
        posX = motionUpdateEvent.getPositionX();
        double y = motionUpdateEvent.getPositionY();
        posZ = motionUpdateEvent.getPositionZ();

        if (!motionUpdateEvent.isLockview())
        {
        ...
        boolean var1 = motionUpdateEvent.isSprinting();
        ...
        boolean var2 = motionUpdateEvent.isSneaking();
        ...
        if (this.func_175160_A())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (var13 && var14)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, y, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround));
                }
                else if (var13)
                {
        ...
        if (!motionUpdateEvent.isLockview())
        {
            rotationYaw = motionUpdateEvent.getOldRotationYaw();
            rotationPitch = motionUpdateEvent.getOldRotationPitch();
        }

        Gun.getInstance().getEventManager().dispatch(new MotionUpdateEvent(MotionUpdateEvent.Time.AFTER));
    }
    public void setLocation(final LocationUtils location) {
        ...
        setBoundingBox(new AxisAlignedBB(this.posX - var7, this.posY, this.posZ - var7, this.posX + var7, this.posY + var8, this.posZ + var7));
    }
    
    public LocationUtils getLocation() {
        return new LocationUtils(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z)
    {
        ...
        Gun.getInstance().getEventManager().dispatch(pushOutOfBlocksEvent);
    ...
    public void onLivingUpdate()
    {
        ...
        {
            ItemInUseEvent itemInUseEvent = new ItemInUseEvent(0.2F);
            Gun.getInstance().getEventManager().dispatch(itemInUseEvent);
            this.movementInput.moveStrafe *= itemInUseEvent.getSpeed();
            this.movementInput.moveForward *= itemInUseEvent.getSpeed();
    ...
    public float getDirection() {
        ...
        final float forward = moveForward;
        final float strafe = moveStrafing;
    ...
    public static float[] postAimWithVec(final Vec3 pos, final EnumFacing facing) {
        final Minecraft mc = Minecraft.getMinecraft();
        final Entity temp = new EntitySnowball(mc.theWorld);
        temp.posX = pos.xCoord + 0.525;
        temp.posY = pos.yCoord + 0.525;
        temp.posZ = pos.zCoord + 0.525;
        mc.theWorld.removeEntity(temp);
