...
public class GuiChat extends GuiScreen
{
    ...
    private String lastTabbedMod = null;
    private List foundPlayerNames = Lists.newArrayList();
    private List<Object> commands = Stream.of(Gun.getInstance().getCommandManager().getRegistry(), Gun.getInstance().getModuleManager().getRegistry()).flatMap(Collection::stream).collect(Collectors.toList());
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException
    {
        ...
        if (keyCode == 15)
        {
            ...
            /*if ((this.inputField.getText().startsWith(CommandManager.prefix) && this.inputField.getText().length() > CommandManager.prefix.length()+1))
            {
                List<Object> collect = commands.stream().filter(o -> {
                    if (o instanceof Module) {
                        return ((Module) o).getLabel().toLowerCase().startsWith(this.inputField.getText().toLowerCase());
                    } else if (o instanceof Command) {
                        for (String alias : ((Command) o).getAliases()) {
                            return alias.toLowerCase().startsWith(this.inputField.getText().toLowerCase());
                        }
                    }
                    return false;
                ...
                if (!collect.isEmpty())
                {
                    if (modIndex >= collect.size())
                    {
                        modIndex = 0;
                    }
                    if (collect.get(modIndex) instanceof Command)
                    {
                        this.inputField.setText(CommandManager.prefix + ((Command) collect.get(modIndex)).getAliases()[0]);
                    } else if (collect.get(modIndex) instanceof Module)
                    {
                        this.inputField.setText(CommandManager.prefix + ((Module) collect.get(modIndex)).getLabel().toLowerCase());
                    }
                    modIndex++;
                }
            }*/
             if (this.inputField.getText().equals(CommandManager.prefix) || (lastTabbedMod != null && this.inputField.getText().endsWith(lastTabbedMod)))
            {
                if (moduleIndex >= commands.size())
                {
                    moduleIndex = 0;
                }
                if (commands.get(moduleIndex) instanceof Command)
                {
                    Command command = (Command)commands.get(moduleIndex);
                    lastTabbedMod = command.getAliases()[0];
                    this.inputField.setText(CommandManager.prefix + command.getAliases()[0].toLowerCase());
                } else if (commands.get(moduleIndex) instanceof Module)
                {
                    Module module = (Module)commands.get(moduleIndex);
                    lastTabbedMod = module.getLabel();
                    this.inputField.setText(CommandManager.prefix + module.getLabel().toLowerCase());
                }
                moduleIndex++;
            } else
    ...
    private void sendAutocompleteRequest(String p_146405_1_, String p_146405_2_)
    {
        if (p_146405_1_.length() >= 1)
        {
            ...
            {
                var3 = this.mc.objectMouseOver.getPos();
            }
