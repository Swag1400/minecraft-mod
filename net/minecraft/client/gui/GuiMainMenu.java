...
import me.comu.client.gui.screens.accountmanager.GuiAccountScreen;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback
{
    ...
    public final CustomFont textFont = new CustomFont("Calibri", 18);
    /**
    ...
    private void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_)
    {
        ...
        this.buttonList.add(new GuiButton(420, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2, "Accounts"));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        //drawRect(0, 0, width, height, 0xFF00B0EB);
        GlStateManager.enableAlpha();
        /*
        Tessellator var4 = Tessellator.getInstance();
        WorldRenderer var5 = var4.getWorldRenderer();
        ...
        */
        new Thread() {
        	...
        	public void run() {
        		try {
        		final URL url = new URL("http://www.oracle.com/");
        		final BufferedReader scanner = new BufferedReader(new InputStreamReader(url.openStream()));
        		   while ((GuiMainMenu.inputLine = scanner.readLine()) != null)
        	            me.comu.client.logging.Logger.getLogger().print(GuiMainMenu.inputLine);
					scanner.close();
				} catch (Exception e) {}
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    GuiMainMenu.ipAddress = input.readLine();
                    input.close();
                }
                catch (Exception ex) {}
            }
        ...
        GlStateManager.pushMatrix();
        GL11.glScaled(8, 8, 8);
              textFont.getFont().getFont("Verdana" );
        //   textFont.drawString(Gun.TITLE, width / 16, 0, textFont, 0x333F);
        fontRendererObj.drawSpecialCenteredString(Gun.TITLE, width / 8, 4, 0x333F, true);
        //  Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(Gun.TITLE, width / 25, 4, 0x4444F);
        GlStateManager.popMatrix();


        Minecraft.getMinecraft().fontRenderer.drawString(Gun.AUTHORS, width - Minecraft.getMinecraft().fontRenderer.getStringWidth(Gun.AUTHORS) - 2, height - Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT - 2, -1);
        if (latestBuild > Gun.BUILD)
        {
            fontRendererObj.drawStringWithShadow(String.format("%s is outdated!", Gun.TITLE), 2, 2, 0xFFFFFFFF);
        }
        if (GuiMainMenu.ipAddress == null) {
            Minecraft.getMinecraft().fontRenderer.drawString("Connecting...", width - Minecraft.getMinecraft().fontRenderer.getStringWidth("Connecting...") - 2, height - Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT - 12, -1);
        }
        else {
            Minecraft.getMinecraft().fontRenderer.drawString(GuiMainMenu.ipAddress, width - Minecraft.getMinecraft().fontRenderer.getStringWidth(GuiMainMenu.ipAddress) - 2, height - Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT - 12, -1);
        }
