...
public class GuiNewChat extends Gui
{
                    ...
                    if (var10 != null)
                    {
                        ...
                        if (var11 < 200 || var3)
                        {
                            ...
                            if (var14 > 3)
                            {
                                ...
                                ShowMessageEvent showMessageEvent = new ShowMessageEvent(var17);
                                Gun.getInstance().getEventManager().dispatch(showMessageEvent);
                                var17 = showMessageEvent.getMessage();
                                GlStateManager.enableBlend();
                                this.mc.fontRenderer.drawStringWithShadow(var17, (float) var15, (float)(var16 - 8), 16777215 + (var14 << 24));
    ...
    public void printChatMessageWithOptionalDeletion(IChatComponent p_146234_1_, int p_146234_2_)
    {
        ...
        String message = p_146234_1_.getFormattedText();

        for (EnumChatFormatting chat : EnumChatFormatting.values())
        {
            switch (chat.name().toLowerCase())
            {
                case "aqua":
                    message = message.replace(chat.toString(), ANSI_CYAN);
                    break;

                case "black":
                    message = message.replace(chat.toString(), ANSI_BLACK);
                    break;

                case "dark_red":
                case "red":
                    message = message.replace(chat.toString(), ANSI_RED);
                    break;

                case "dark_blue":
                case "blue":
                    message = message.replace(chat.toString(), ANSI_BLUE);
                    break;

                case "gold":
                case "yellow":
                    message = message.replace(chat.toString(), ANSI_YELLOW);
                    break;

                case "dark_purple":
                case "light_purple":
                    message = message.replace(chat.toString(), ANSI_PURPLE);
                    break;

                case "dark_green":
                case "green":
                    message = message.replace(chat.toString(), ANSI_GREEN);
                    break;

                case "white":
                    message = message.replace(chat.toString(), ANSI_WHITE);
                    break;

                case "italic":
                    message = message.replace(chat.toString(), ANSI_ITALIC);
                    break;

                case "underline":
                    message = message.replace(chat.toString(), ANSI_UNDERLINE);
                    break;

                case "bold":
                    message = message.replace(chat.toString(), ANSI_HIGH_INTENSITY);
                    break;

                case "strikethrough":
                    message = message.replace(chat.toString(), ANSI_STRIKETHROUGH);
                    break;

                case "reset":
                    message = message.replace(chat.toString(), ANSI_RESET);
                    break;
            }
        }

        logger.info("[CHAT] " + StringUtils.stripControlCodes(message));
    }
    ...
    private void setChatLine(IChatComponent p_146237_1_, int p_146237_2_, int p_146237_3_, boolean p_146237_4_)
    {
        ...
        List var6 = GuiUtilRenderComponents.func_178908_a(p_146237_1_, var5, this.mc.fontRenderer, false, false);
                ...
                if (var6 <= MathHelper.floor_float((float) this.getChatWidth() / this.getChatScale()) && var7 < this.mc.fontRenderer.FONT_HEIGHT * var8 + var8)
                {
                    ...
                    if (var9 >= 0 && var9 < this.field_146253_i.size())
                    {
                        ...
                        while (var12.hasNext())
                        {
                            ...
                            {
                                var11 += this.mc.fontRenderer.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText) var13).getChatComponentText_TextValue(), false));
