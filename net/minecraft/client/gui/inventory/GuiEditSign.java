...
public class GuiEditSign extends GuiScreen {
    ...
    public void onGuiClosed() {
        ...
        NetHandlerPlayClient var1 = this.mc.func_175102_a();
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if (GuiScreen.func_175279_e(keyCode)) {
            String copy = GuiScreen.getClipboardString();
            if (!copy.equalsIgnoreCase("")) {
                var3 += copy.substring(0, Math.min(copy.length(), 15));
            }
            var3 = var3.substring(0, Math.min(var3.length(), 15));
        }
