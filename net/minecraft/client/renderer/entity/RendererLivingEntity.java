...
import me.comu.client.module.impl.toggle.render.Tracers;
...
public abstract class RendererLivingEntity extends Render {
    ...
    public void doRender(EntityLivingBase p_76986_1_, double p_76986_2_, double p_76986_4_, double p_76986_6_, float p_76986_8_, float p_76986_9_) {
        AntiLagEvent antiLagEvent = new AntiLagEvent();
        Gun.getInstance().getEventManager().dispatch(antiLagEvent);
        if (antiLagEvent.isAnimals()) {
            if (p_76986_1_ instanceof EntityAnimal || p_76986_1_ instanceof EntityIronGolem || p_76986_1_ instanceof EntitySquid || p_76986_1_ instanceof EntityVillager) {
                return;
            }
        }
        if (antiLagEvent.isMonsters()) {
            if (p_76986_1_ instanceof EntityMob || p_76986_1_ instanceof EntityBat || p_76986_1_ instanceof EntityDragon || p_76986_1_ instanceof EntityGhast || p_76986_1_ instanceof EntitySlime) {
                return;
            }
        }
        if (antiLagEvent.isPlayers())
        {
            if (!p_76986_1_.equals(ClientUtils.player()) && p_76986_1_ instanceof EntityPlayer)
            {
                return;
            }
        }
        RenderEntityEvent renderEntityEvent = new RenderEntityEvent(RenderEntityEvent.Time.BEFORE);
        Gun.getInstance().getEventManager().dispatch(renderEntityEvent);
        ...
        try {
            ...
            float var20 = (p_76986_1_ == Minecraft.getMinecraft().thePlayer) ? Minecraft.getMinecraft().thePlayer.clientEventPitch : (p_76986_1_.prevRotationPitch + (p_76986_1_.rotationPitch - p_76986_1_.prevRotationPitch) * p_76986_9_);
            ...
            if (this.field_177098_i) {
                ...
                Tracers tracers = (Tracers) Gun.getInstance().getModuleManager().getModuleByAlias("tracers");
                EnumProperty<Tracers.Mode> mode = (EnumProperty) tracers.getPropertyByAlias("Mode");

                if (!p_76986_1_.equals(Minecraft.getMinecraft().thePlayer) && (tracers != null && tracers.isRunning() && mode.getValue() == Tracers.Mode.OUTLINE && Tracers.esp.getValue() && tracers.isValidEntity(p_76986_1_))) {
                    ...
                    this.renderModel(p_76986_1_, var17, var16, var14, var12, var20, 0.0625F);
                    tracers.renderThree();
                    tracers.renderFour(p_76986_1_);
                    this.renderModel(p_76986_1_, var17, var16, var14, var12, var20, 0.0625F);
                    tracers.renderFive();
                } else if (!p_76986_1_.equals(Minecraft.getMinecraft().thePlayer) && (tracers != null && tracers.isRunning() && (mode.getValue() == Tracers.Mode.FILL || mode.getValue() == Tracers.Mode.SHADERS || mode.getValue() == Tracers.Mode.RECTFILL) && Tracers.esp.getValue() && tracers.isValidEntity(p_76986_1_))) {
                    int renderColor = ColorHelper.getColor(245, 37, 58);
                    Minecraft mc = Minecraft.getMinecraft();
                    if (p_76986_1_.hurtResistantTime > 15 && mode.getValue() == Tracers.Mode.FILL)
                        renderColor = ColorHelper.getColor(1, 0, 0, 1);
                    if (Gun.getInstance().getFriendManager().isFriend(p_76986_1_.getName()) && !(p_76986_1_ instanceof EntityPlayerSP))
                        renderColor = ColorHelper.getColor(0, 195, 255, 255);
                    if (Gun.getInstance().getEnemyManager().isEnemy(p_76986_1_.getName()) && !(p_76986_1_ instanceof EntityPlayerSP))
                        renderColor = ColorHelper.getColor(255, 51, 192, 255);
                    if (Gun.getInstance().getStaffManager().isStaff(p_76986_1_.getName()) && !(p_76986_1_ instanceof EntityPlayerSP))
                        renderColor = ColorHelper.getColor(255, 225, 51, 255);
                    mc.entityRenderer.disableLightmap();
                    RenderMethods.glColor(renderColor);
                    GL11.glPushMatrix();
                    GL11.glBlendFunc(770, 771);
                    GL11.glDisable(3553);
                    if (mode.getValue() != Tracers.Mode.SHADERS)
                    ...
                    RenderMethods.glColor(-1);

                  /*  Minecraft.getMinecraft().entityRenderer.disableLightmap();
                    if (p_76986_1_.hurtResistantTime > 15)
                        GlStateManager.color(0.9411765f, 0.5019608f, 0.5019608f);
                    else
                    GlStateManager.color(0.49803922f, 1.0f, 0.83137256f);
                    GL11.glPushMatrix();
                    GL11.glBlendFunc(770, 771);
                    GL11.glDisable(3553);
                    RenderHelper.disableStandardItemLighting();
        ...
        RenderEntityEvent renderEntityEvent1 = new RenderEntityEvent(RenderEntityEvent.Time.AFTER);
        Gun.getInstance().getEventManager().dispatch(renderEntityEvent1);
    }
    ...
    protected boolean canRenderName(EntityLivingBase targetEntity) {
        ...
        if (targetEntity instanceof EntityPlayer && targetEntity != var2) {
            PassSpecialRenderEvent passSpecialRenderEvent = new PassSpecialRenderEvent();
            Gun.getInstance().getEventManager().dispatch(passSpecialRenderEvent);
            if (passSpecialRenderEvent.isCancelled())
                return false;
