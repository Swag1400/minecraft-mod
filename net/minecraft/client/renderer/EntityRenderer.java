...
import me.comu.client.module.impl.toggle.combat.Reach;
import me.comu.client.module.impl.toggle.render.ViewClip;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float p_78473_1_) {
        ...
        if (var2 != null && this.mc.theWorld != null) {
            ...
            Reach reach = (Reach) Gun.getInstance().getModuleManager().getModuleByAlias("Range");
            if (reach != null && reach.isRunning()) {
                var3 = (double) reach.getPropertyByAlias("Reach").getValue();
            }
    ...
    private void updateFovModifierHand() {
        float var1 = 1.0F;
        ViewmodelEvent viewmodelEvent = new ViewmodelEvent(ViewmodelEvent.Type.FOV);
        Gun.getInstance().getEventManager().dispatch(viewmodelEvent);

        if (viewmodelEvent.isNoFov()) {
            return;
        }
    ...
    private void hurtCameraEffect(float p_78482_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            RenderGameOverlayEvent renderGameOverlayEvent = new RenderGameOverlayEvent(RenderGameOverlayEvent.Type.HURTCAM);
            Gun.getInstance().getEventManager().dispatch(renderGameOverlayEvent);

            if (renderGameOverlayEvent.isRenderFire()) {
                return;
            }
        ...
        if (var2 instanceof EntityLivingBase && ((EntityLivingBase) var2).isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera, this.mc.theWorld, var7, var8, var2);
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int var17 = 0; var17 < 8; ++var17) {
                    ...
                    if (var21 != null) {
                        ...
                        ViewClip viewclip = (ViewClip) Gun.getInstance().getModuleManager().getModuleByAlias("viewclip");
                        if (var22 < var11 && !viewclip.isRunning()) {
                            var11 = var22;
                        }
                        // TODO:
                    }
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (var4 > 0.0F) {
            ...
            var6 *= var6;
            NauseaEvent NauseaEvent = new NauseaEvent();
            Gun.getInstance().getEventManager().dispatch(NauseaEvent);
            if (!NauseaEvent.isCancelled()) {
        ...
        if (this.lightmapUpdateNeeded) {
            ...
            if (var2 != null) {
                ...
                for (int var3 = 0; var3 < 256; ++var3) {
                    ...
                    float var17;
                    NightVisionEvent nightVisionEvent = new NightVisionEvent(NightVisionEvent.Type.VISUAL);
                    Gun.getInstance().getEventManager().dispatch(nightVisionEvent);

                    if (this.mc.thePlayer.isPotionActive(Potion.nightVision) || nightVisionEvent.isCancelled()) {
                    ...
                    GammaSettingEvent gammaSettingEvent = new GammaSettingEvent(mc.gameSettings.gammaSetting);
                    Gun.getInstance().getEventManager().dispatch(gammaSettingEvent);
                    var16 = gammaSettingEvent.getGammaSetting();
                    var17 = 1.0F - var13;
    ...
    private float func_180438_a(EntityLivingBase p_180438_1_, float partialTicks) {
        NightVisionEvent nightVisionEvent = new NightVisionEvent(NightVisionEvent.Type.TIME);
        Gun.getInstance().getEventManager().dispatch(nightVisionEvent);
        int var3 = !nightVisionEvent.isCancelled() ? p_180438_1_.getActivePotionEffect(Potion.nightVision).getDuration() : 645645645;
        ...
        if (!this.field_175073_D) {
            ...
            if (var2 && !((EntityPlayer) var1).capabilities.allowEdit) {
                ...
                if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                    BlockPos var4 = this.mc.objectMouseOver.getPos();
    ...
    private void func_175068_a(int pass, float partialTicks, long finishTimeNano) {
        ...
        Gun.getInstance().getEventManager().dispatch(new RenderEvent(partialTicks));

        if (!renderFirstPersonHand) {
            GlStateManager.clear(256);
            RenderHandEvent renderHandEvent = new RenderHandEvent();
            Gun.getInstance().getEventManager().dispatch(renderHandEvent);
    ...
    private void updateFogColor(float partialTicks) {
        ...
        Gun.getInstance().getEventManager().dispatch(blindnessEvent);
        if (!blindnessEvent.isCancelled() && var3 instanceof EntityLivingBase && ((EntityLivingBase) var3).isPotionActive(Potion.blindness)) {
        ...
        if (var3 instanceof EntityLivingBase && (((EntityLivingBase) var3).isPotionActive(Potion.nightVision) || nightVisionEvent.isCancelled())) {
    ...
    private void frameInit() {
        ...
        if (world != null && !Config.ran) {
            me.comu.client.logging.Logger.getLogger().printToChat(String.format("Welcome &e%s&7!", System.getProperty("user.name")));
            me.comu.client.logging.Logger.getLogger().printToChat(String.format("Use &e\"%shelp\"&7 to get started.", Gun.getInstance().getCommandManager().getPrefix()));
            Config.ran = true;
        }
    ...
    private void updateMainMenu(GuiMainMenu mainGui) {
        try {
            ...
            if (day == 18 && month == 6) {
                e = "Happy birthday, Comu!";
            }

            if (day == 15 && month == 6) {
                e = "Happy birthday, Gringo!";
            }
