...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public float func_180471_a(Block p_180471_1_)
    {
        ...
        MiningSpeedEvent miningSpeedEvent = new MiningSpeedEvent();
        Gun.getInstance().getEventManager().dispatch(miningSpeedEvent);

        if (this.isPotionActive(Potion.digSpeed))
        {
            var2 *= 1.0F + (float)(this.getActivePotionEffect(Potion.digSpeed).getAmplifier() + 1) * 0.2F;
        }
        else if (miningSpeedEvent.isCancelled())
        {
            var2 *= (miningSpeedEvent.getSpeed());
        }
                ...
                if (var2 > 0.0F || var4 > 0.0F)
                {
                    ...
                    if (var14)
                    {
                        if (var18 > 0)
                        {
                            ...
                            SprintingAttackEvent sprintingAttackEvent = new SprintingAttackEvent();
                            Gun.getInstance().getEventManager().dispatch(sprintingAttackEvent);

                            if (!sprintingAttackEvent.isCancelled())
                            {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
    ...
    {
//        EntityKillEvent entityKillEvent = new EntityKillEvent();
//        Gun.getInstance().getEventManager().dispatch(entityKillEvent);
...
//            return;
    ...
    {
        WaterMoveEvent waterMoveEvent = new WaterMoveEvent();
        Gun.getInstance().getEventManager().dispatch(waterMoveEvent);

        if (waterMoveEvent.isCancelled())
        {
            return false;
        }
